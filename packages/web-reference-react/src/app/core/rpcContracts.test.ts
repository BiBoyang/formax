...
const COMPRESSION_GOLDEN_PENDING_RESTORE = {
  ...
  restoreDiagnostics: {
    ...
    source: 'session_memory_sidecar' as const,
...
describe('rpcContracts', () => {
          ...
          diagnostics: {
            ...
            nextTurnFixed: {
              ...
              workingSetSignals: {
                ...
                keepMinUserTurnsBoost: 1,
                taskStateKinds: ['recent_files', 'plan_state', 'mode_state'],
                selectionReasons: ['recent_files'],
                anchorKind: 'task-cluster-oops',
                anchorToolNames: ['Read'],
                anchorBacktrackTurns: 0,
                anchorMaxBacktrackTurns: 1,
              },
  ...
  it('accepts legacy workingSetSignals payloads without v5 fields', () => {
    expect(
      parseTurnStartLikeResponse({
        turn: { id: 'turn-1' },
        local: {
      ...
      }).localDiagnostics?.nextTurnFixed.workingSetSignals,
    ).toEqual({
      recentFileCount: 1,
      hasPlanState: true,
      hasTodoState: false,
