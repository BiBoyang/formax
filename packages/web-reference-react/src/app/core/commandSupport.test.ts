...
describe('commandSupport', () => {
  it('supports only the five allowed slash commands for web', () => {
    expect(getWebSupportedSlashCommands().sort()).toEqual(['/clear', '/compact', '/context', '/init', '/todos'])
    ...
    expect(isWebSupportedCommand('/context')).toBe(true)
  ...
  it('exposes slash command specs with descriptions in stable order', () => {
    ...
    expect(specs.map((spec) => spec.command)).toEqual(['/init', '/clear', '/compact', '/context', '/todos'])
