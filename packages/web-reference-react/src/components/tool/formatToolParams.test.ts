...
describe('formatToolParams', () => {
  ...
  it('maps grep/search pattern + path + output_mode', () => {
    ...
    expect(search).toEqual([
      ...
      { label: 'path', value: 'packages/web-reference-react/src', valueType: 'string' },
    ])
  })
  ...
  it('formats absolute file paths relative to cwd', () => {
    const params = formatToolParams({
      toolName: 'Read',
      paramsText: 'file_path="/Users/david/Documents/github/formax/packages/web-reference-react/src/App.tsx"',
      cwd: '/Users/david/Documents/github/formax',
    })

    expect(params).toEqual([
      { label: 'file', value: 'packages/web-reference-react/src/App.tsx', valueType: 'string' },
    ])
  })
