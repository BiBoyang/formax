function stripTrailingSystemReminderBlock(raw: string): string {
  ...
  const tail = value.slice(markerIndex + 2)
  const closeMarker = '</system-reminder>'
  const closeIndex = tail.lastIndexOf(closeMarker)
  if (closeIndex < 0) return value

  const after = tail.slice(closeIndex + closeMarker.length)
  if (after.trim().length !== 0) return value
