...
describe('SetupWizard', () => {
  ...
  it('supports anthropic vendor presets and custom placeholder', async () => {
    ...
    await waitForText(first.lastFrame, '❯ DeepSeek')
    first.stdin.write('\r')
    await waitForText(first.lastFrame, 'https://api.deepseek.com/anthropic')
    first.unmount()
    ...
    second.stdin.write('\u001B[B')
    await waitForText(second.lastFrame, '❯ GLM')
    second.stdin.write('\r')
    await waitForText(second.lastFrame, 'https://open.bigmodel.cn/api/anthropic')
    second.unmount()

    const third = renderSetupWizard()
    await goToProvider(third.lastFrame)
    third.stdin.write('\r')
    await waitForText(third.lastFrame, 'Select Anthropic-compatible provider')
    third.stdin.write('\u001B[B')
    third.stdin.write('\u001B[B')
    third.stdin.write('\u001B[B')
    third.stdin.write('\u001B[B')
    third.stdin.write('\u001B[B')
    await waitForText(third.lastFrame, '❯ Custom')
    third.stdin.write('\r')
    await waitForText(third.lastFrame, 'https://your-provider.example.com/anthropic')
  })
