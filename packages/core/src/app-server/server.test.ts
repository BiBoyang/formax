...
describe('AppServer', () => {
  ...
  it('patches thread runtime preferences after durable store success and emits runtime-state notification', async () => {
    const notifications: Array<{ jsonrpc: '2.0'; method: string; params?: unknown }> = []
    const server = new AppServer({
      info: { name: 'formax', version: 'test' },
      ...
      threadStore: {
        ...
        async patchThreadRuntimeState(params) {
      ...
      resolveEffectiveRuntimeProfileSummary: async ({ preferences }) => ({
        ...
        baseUrl: '',
        authRef: 'default',
        activeTier: preferences?.modelTier ?? 'sonnet',
        model: 'claude-opus',
  ...
  it('returns replay preferences without synthesizing default runtime state', async () => {
    ...
    const out = await server.handleMessage(request(2, 'thread/replay', { threadId: 'thread-1' }))
