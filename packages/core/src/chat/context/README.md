...
- 当前 auto compact 的 `keep_combo` 已开始根据 working-set signals 做 v4 调整：除了 recent files、plan/todo state、以及 mode state 的动态 boost，working-set anchor 现在也会识别最近成功的 filesystem tool cluster（`Read` / `Grep` / `Glob`），并按 anchor kind 区分 rewind window（`Read=1`，`filesystem_cluster=2`）；`/context` 会通过 `nextTurnFixed.workingSetSignals` / `Working-set signals` 小节显式说明 `anchorKind`、`anchorToolNames`、实际 `anchorBacktrackTurns` 与当前 `anchorMaxBacktrackTurns`
...
- history 重建：`packages/core/src/chat/context/compact.ts`（tail 选择、boundary metadata、preserved-segment metadata、rehydration 拼装、continuation view helper；当前 working-set v4 已覆盖最近成功的 filesystem cluster anchor，并把 recent files、plan/todo state、mode state 合并进 auto keep strategy）
