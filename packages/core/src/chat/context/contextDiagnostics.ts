...
export function analyzeNextTurnFixedContext(args: {
  ...
  return {
    ...
    microCompactImpact: {
      ...
      cacheAwareEligibleToolNames: stack.facts.microCompact.impact.cacheAwareEligibleToolNames,
      cacheAwareMinResultChars: stack.facts.microCompact.impact.cacheAwareMinResultChars,
      ...
      timeAwareEligibleToolNames: stack.facts.microCompact.impact.timeAwareEligibleToolNames,
      timeAwareMinResultChars: stack.facts.microCompact.impact.timeAwareMinResultChars,
...
export function formatContextDiagnosticsReport(args: {
  ...
  const lines = [
    ...
    `- Microcompact time-aware eligible tools: ${formatToolNames(args.nextTurn?.microCompactImpact.timeAwareEligibleToolNames ?? [])}`,
    `- Microcompact time-aware minimum chars: ${formatInt(args.nextTurn?.microCompactImpact.timeAwareMinResultChars ?? 0)}`,
    `- Microcompact time-aware stale user turns: ${formatInt(args.nextTurn?.microCompactImpact.timeAwareMinStaleUserTurns ?? 0)}`,
    `- Microcompact time-aware compacted blocks: ${formatInt(args.nextTurn?.microCompactImpact.timeAwareCompactedBlocks ?? 0)}`,
    `- Microcompact time-aware compacted tools: ${formatToolNames(args.nextTurn?.microCompactImpact.timeAwareToolNames ?? [])}`,
