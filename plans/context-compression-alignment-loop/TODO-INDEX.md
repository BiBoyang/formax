...
- `CCA-004` 引入自适应 microcompact 策略
  - 目标：根据 token 压力、tool 类型、结果大小、时间远近动态决定压缩范围
  - 验收：不同压力档位下，压缩数量与保留数量有可预测变化

- `CCA-005` 设计并实现 `Bash` / `WebFetch` 安全压缩策略
  - 目标：只压“可重放”结果，避免压掉不可稳定复现的证据
  - 验收：有明确 allow/deny 规则与测试覆盖
