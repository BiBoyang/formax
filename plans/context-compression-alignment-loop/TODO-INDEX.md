...
1. 回到 WebGPT / subagent 审查收敛修复清单，复核是否还有实质未完成项。
2. 回到 Claude Code cache editing microcompact 迁移清单，复核是否只剩 payload-level / smoke-level 验证。
3. 完成当前 WebGPT/cache-editing 主线收口后，再决定是否恢复 `CCA-180`。
4. 恢复 `CCA-180` 后才继续 `CCA-181` preserved-segment relink parity 与 `CCA-182` reactive compact shaping v3。
