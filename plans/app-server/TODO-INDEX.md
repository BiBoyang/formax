...
- [ ] N34 baseline replay 双请求路径的 invariant 日志去重 fixture
  - 目标：覆盖 `hasGap=true 且无 projection -> baseline replay` 两次请求路径，确保 invariant warn 只记一次。
  - 验收：
    - replayThreadEvents fixture 断言同一 replay 调用内仅一次 warn。
    - 不影响现有 hasGap/projection fixture。
