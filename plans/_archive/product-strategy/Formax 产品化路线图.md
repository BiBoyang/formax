...
| 45. 可发布可维护：兼容/降级策略                    | Codex 有清晰配置项（如自定义 provider、base_url、wire_api），利于兼容多环境。                                                                                                                                                                                            | Formax 当前 anthropic-only 且 env 命名带 `2`（`ANTHROPIC_API_KEY2`）。                                                                                                                       | 命名不直观、迁移成本高                               | 1) 统一 env 命名（FORMAX_ 前缀） 2) 兼容旧 env（迁移提示） 3) 写 migration 文档与 `/doctor` 检测                                             |                                                                                                          |
...
* **MVP 步骤**
  ...
  5. 兼容旧 env：`ANTHROPIC_API_KEY2/BASE_URL2` → 给 warning（一次性）
* **落点**
