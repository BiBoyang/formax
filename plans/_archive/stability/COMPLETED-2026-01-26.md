...
- useReplController：reset 逻辑抽为 `resetStreamingBuffers()` / `resetSessionState()`（仅重构，不改行为）：见 `src/features/repl/useReplController.ts`、`src/features/repl/useReplController.test.tsx`
