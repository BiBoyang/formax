...
- `Query` 已支持 `interrupt()`、`close()`、`initializationResult()`、`supportedCommands()`、`supportedAgents()`、`supportedModels()`、`accountInfo()`、`stopTask()`（中止当前 query）与 `setModel()`、`setPermissionMode()`、`setMaxThinkingTokens()`；`mcpServerStatus()/setMcpServers()/reconnectMcpServer()/toggleMcpServer()/streamInput()/rewindFiles()` 当前仍显式报未支持；其余 `Query` 对象方法（`supported*`/`mcp*` 等）暂不纳入一期范围。
...
- `setMcpServers()/rewindFiles()` 在当前能力下仍显式报未支持，但返回类型已与官方同名结果类型对齐；`stopTask()` 当前对齐为中止式兼容语义。
...
| `agent`/`agents` | `options.agent` / `options.agents` | Partial | 已支持：在 in-process SDK 模式下作为兼容 no-op 参数接受。 |
