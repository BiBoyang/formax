...
- `supportedModels()` 返回模型项同时包含 `value/displayName/supportsEffort/supportsAdaptiveThinking`（官方常用）与 `model/provider/supports_reasoning_effort`（Formax 兼容）字段；活动模型回填项在能力可推导时会补齐 effort 兼容字段。
