...
| `Query` 返回对象 | `AsyncGenerator + interrupt/setModel/...` 方法集 | 返回 `Query`（`AsyncGenerator<QueryMessage>` + `interrupt()`）；其余方法暂未支持 | Partial |

说明：
- 当前已支持 `AsyncIterable<SDKUserMessage>`（user/text 子集）。
- `Query` 已支持 `interrupt()`；其余 `Query` 对象方法（`setModel`/`supported*`/`mcp*` 等）暂不纳入一期范围。
