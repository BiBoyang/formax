...
- `query(args): Query` (`AsyncGenerator<QueryMessage, void, unknown>` + `interrupt()` + `close()` + control methods)
...
- Query control handle (`query(...).interrupt()` / `query(...).close()` + `setModel/setPermissionMode/setMaxThinkingTokens` pre-start overrides)
