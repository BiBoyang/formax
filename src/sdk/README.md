...
- `query(args): Query` (`AsyncGenerator<QueryMessage, void, unknown>` + `interrupt()` + `close()` + `initializationResult()` + control methods)
...
- Query control handle (`query(...).interrupt()` / `query(...).close()` + `initializationResult()` + `setModel/setPermissionMode/setMaxThinkingTokens` pre-start overrides)
