...
| `supportedModels` | `supportedModels` | Supported | 返回 provider 可用模型子集，包含 `value/displayName/supportsEffort/supportsAdaptiveThinking` 等官方常用兼容字段；活动模型回填项在可推导时补齐 effort 与 `max_tokens/contextWindowTokens` 兼容字段。 |
