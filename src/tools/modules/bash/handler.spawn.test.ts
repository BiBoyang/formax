    ...
    return {
      ...
      spawn: () => {
        ...
        queueMicrotask(() => {
          ...
          for (const cb of listeners.get('close') ?? []) cb(0, null)
        })
...
describe('BashToolHandler spawn branches', () => {
  it('returns background task error when spawn emits error', async () => {
    const { createBashToolHandler } = await loadHandlerWithSpawnPlan({ emitError: 'spawn fail' })
