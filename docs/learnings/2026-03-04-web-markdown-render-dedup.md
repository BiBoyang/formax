...
- Updated `apps/web-reference-react/src/app/core/markdownService.ts`:
...
- Updated `apps/web-reference-react/src/components/MarkdownRenderer.tsx`:
...
- Updated tests:
  - `apps/web-reference-react/src/app/core/markdownService.test.ts` adds cross-key hash-cache reuse coverage.
  - `apps/web-reference-react/src/components/MarkdownRenderer.test.tsx` keeps worker-error fallback coverage while avoiding false coupling with cross-key cache reuse.
