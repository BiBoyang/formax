...
Within durable projection replay, compact preserved-segment relink MUST run before durable snip removal and durable collapse replacement. This keeps snip/collapse ranges in the model-facing baseline coordinate space after boundary continuation has recovered any verified preserved tail. Relink is model-facing only and MUST NOT mutate raw transcript, UI scrollback, or persisted JSONL rows. If relink skips because the preserved segment cannot be uniquely and contiguously verified, downstream durable stages MUST still rely on their own coordinate / identity / fingerprint guards; relink status MUST NOT become a new downstream authority.
...
If a provider rejects the initial request for context overflow after request-time collapse has produced a durable commit candidate, the runtime MUST attempt to drain that pending collapse commit before running reactive full compact. This keeps deterministic request collapse from being lost when the overflow path falls through to a heavier materializing compact/retry. Drainage persistence failures SHOULD be isolated from overflow recovery; they MUST NOT suppress an otherwise eligible reactive compact retry.
...
`CSS-312`
Reactive compact fallback is a send-path retry policy, not a middle-layer reducer. Interactive TUI and app-server turn paths MUST attempt it after a real provider request fails with an eligible context-overflow signal. SDK query paths currently MUST remain fail-fast by default unless a future SDK contract adds an explicit opt-in retry option. Abort-like errors MUST preserve abort semantics even if their message contains overflow-like text, and auth / rate-limit signals MUST NOT be coerced into reactive compact triggers even when mixed with overflow-like wording. A successful fallback preparation MUST retry at most once from the reactive-prepared `history`, `requestHistory`, `requestUser`, and request-side `cacheEditPlan`; cache edits from the failed oversized request MUST NOT be reused blindly. Best-effort pre-retry drainage of pending durable compression state MUST NOT make the retry path less recoverable when session persistence fails. If the retry also fails, including another overflow-like failure, the runtime MUST NOT start a second reactive compact loop.
...
`CSS-404a`
When `/context` diagnostics expose durable tool-result content replacement facts, they MUST expose bounded projection metadata only. Diagnostics MAY report status, applied/skipped counts, tool-use identity, replacement-content length, source scope, generation/fingerprint fields, and skipped/drift reason; they MUST NOT dump full `replacementContent` by default.
