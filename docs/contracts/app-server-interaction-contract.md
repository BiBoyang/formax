...
| `turn/modeChanged`（运行期 mode 同步）                                       | `src/app-server/turnRunner.ts`, `src/features/semantics/core/replModeTransition.ts`                                           | `src/app-server/turnRunner.test.ts`, `packages/web-reference-react/src/App.test.tsx`                                         |
